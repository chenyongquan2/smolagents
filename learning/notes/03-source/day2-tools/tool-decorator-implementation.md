...
> 💡 联系 [python-class-vs-instance-attributes.md](../python-prep/python-class-vs-instance-attributes.md) §6 —— smolagents 全程把 schema 设计成**类属性**，连动态构造时也是直接挂在 `SimpleTool.xxx` 上而不是在 `__init__` 里 `self.xxx`。一致性贯彻得很彻底。
...
- 横向关联：
  - [python-class-vs-instance-attributes.md](../python-prep/python-class-vs-instance-attributes.md) — 为什么 `@tool` 也用类属性挂 schema
