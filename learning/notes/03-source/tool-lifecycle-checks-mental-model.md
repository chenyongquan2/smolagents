...
> 读完本笔记建立直觉认识后，再去看 [tool-validation-three-layers.md](tool-validation-three-layers.md) 的实现细节会**轻松很多**。
>
...
**回顾 [python-init-subclass.md](python-init-subclass.md)** 讲过的：基类 `__init__` 校验**子类可能忘调 `super().__init__()`** 就跳过了。所以必须用 `__init_subclass__` wrap 子类 `__init__` —— 让校验**绕不过**。
...
- **第 ① 项用的是 `getattr(self, attr, None)` 而不是 `self.attr`**：因为 Tool 基类只给了 4 个**类属性注解**（[tools.py:131-134](../../../src/smolagents/tools.py#L131)），子类没赋值时这些属性根本不存在（详见 [python-class-and-dataclass.md](python-class-and-dataclass.md) "Python 没有字段声明"）。直接 `self.inputs` 会崩 AttributeError，错误信息没"You must set ..."友好。
...
**`forward()`** 默认抛 `NotImplementedError` —— 软约束，详见 [python-abc-abstract-base-class.md](python-abc-abstract-base-class.md) §4。
...
- 横向关联：
  - [python-init-subclass.md](python-init-subclass.md) — wrap `__init__` 的机制（出厂质检挂钩怎么装的）
  - [python-abc-abstract-base-class.md](python-abc-abstract-base-class.md) §4 — 为什么 forward 用软约束（给 wrapper 子类留绕过口子）
