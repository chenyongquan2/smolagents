...
## 4. ⭐ 为什么 `Model` 的构造函数用 `**kwargs` 不用 `*args`？
...
| 位置 | 写法 | 用途 |
|---|---|---|
| [models.py:484 `Model` 构造函数](../../../../src/smolagents/models.py#L484) | `**kwargs` 存到 `self.kwargs` | 保存"非核心、不可预知" LLM 配置参数；最高优先级覆盖 HTTP body |
...
>
> 三层都用 dict.update 合并，最后用 `REMOVE_PARAMETER` 哨兵删字段。详见 [model-class-role-overview.md §3](../day3-models/model-class-role-overview.md)。
...
- 源码上下文：
  - [model-class-role-overview.md](../day3-models/model-class-role-overview.md) — Model 基类角色 + `self.kwargs` 三层优先级机制
