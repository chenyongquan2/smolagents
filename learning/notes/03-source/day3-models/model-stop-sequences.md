...
> 💡 **Day 1 [planning-mechanics.md](../day1-memory/planning-mechanics.md)** 早就提到："prompt 工程是双向控制：YAML 模板规定结构 + stop_sequences 强制截断 LLM 跑题"。本笔记是这条结论的源码侧验证。
...
| ③ 用户主动删字段 | `Model(stop=REMOVE_PARAMETER)` （详见 [python-sentinel-pattern.md](../python-prep/python-sentinel-pattern.md)） | 用户显式让 body 里**根本没有** stop 字段 | 已知 provider 不接受 stop 字段（连 null 也不行） |
...
详见 [python-sentinel-pattern.md](../python-prep/python-sentinel-pattern.md)。
...
| 关联点 | 在哪里出现 | 链接 |
|---|---|---|
| PlanningStep 用 `<end_plan>` 兜底 | Day 1 实证实验 | [planning-mechanics.md](../day1-memory/planning-mechanics.md) |
| prompt 模板严格规定结构 | Day 1 段 4 洞察 | [planning-mechanics.md](../day1-memory/planning-mechanics.md) |
...
- 关联 Day 1：
  - [planning-mechanics.md](../day1-memory/planning-mechanics.md) — `<end_plan>` 在 PlanningStep 的实证
- 关联 Week 1：
...
- 关联 Day 3：
  - [python-sentinel-pattern.md](../python-prep/python-sentinel-pattern.md) — `REMOVE_PARAMETER` 哨兵机制（防线 ③ 的实现基础）
