...
回顾 Day 2 [tool-schema-rendering-mental-model.md](tool-schema-rendering-mental-model.md) §4 讲的：HTTP `tools` 字段不是 `Tool.to_tool_calling_prompt` 渲染的，是由 [models.py:288 `get_tool_json_schema`](../../../src/smolagents/models.py#L288) 渲染的。**Day 3 看到它在哪里被调到** —— 就是这里，line 540。
...
> 💡 这个 "**最早知道 → 最先写入 → 最容易被覆盖**" 的反直觉设计，其实是表达"**用户意图最高 vs 框架默认最低**"的优先级哲学。Day 2 [tool-class-role-overview.md](tool-class-role-overview.md) §5 讲的"每件事尽可能放在能做它的最早时机"是**写入时机**层面；这里 §4 的优先级是**覆盖关系**层面，**两者互不冲突**。
...
- Day 2 闭环：
  - [tool-schema-rendering-mental-model.md](tool-schema-rendering-mental-model.md) — Day 2 预言"HTTP tools 字段在 Tool 类外渲染"
- Week 1 闭环：
  - [chat-message-roles.md](../02-concepts/chat-message-roles.md) — role 交替要求 + chat template 原理
- Python 预习：
  - [python-args-kwargs.md](python-args-kwargs.md) — 三层 kwargs 合并依赖的语法基础
- Day 3 后续：
