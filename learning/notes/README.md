...
- ⭐ [tools-are-python-callables.md](02-concepts/tools-are-python-callables.md) — **两种 agent 调用的工具本质都是 Python callable**（Day 2 + Day 5 跨日洞察单独抽出）。一句话定调（差异不在工具本质，在"谁解析参数 + 谁触发调用"）+ 同一条调用链图（两种 agent 最终都汇到 `Tool.__call__ → Tool.forward(**args)`）+ 异构来源 6 类（本地 Python / MCP / Gradio / HF Hub / LangChain / managed_agent）都被规范成 Python callable + CodeAgent 远程沙箱边界（e2b / docker / wasm + `send_tools` 抽象方法）+ 设计胜利反例对比（每种工具来源各管各 vs 统一 Tool 抽象）+ Week 1 / Day 2 / Day 5 闭环回收 8 处。**Tool 类作为"通用插槽"是 smolagents 整个生态扩展性的根 —— Week 3 写自定义 tool 时回头看**
...
**Day 5 · agents.py 下半 ⭐ 系列**（**仓库的"心脏"，严格按顺序读：00 → 01 → 02 → 03 → 04 → self-check**）：
- ⭐⭐ 00 [step-stream-role-overview.md](03-source/day5-step-stream/00-step-stream-role-overview.md) — **`_step_stream` mental model 骨架（Day 5 必读起点）**。一句话定调（ReAct 一步的实际承担者）+ 谁调谁（Day 4 外层 `_run_stream` for 循环 → 子类 _step_stream）+ ⭐⭐ **5 个固定动作**（read messages / call LLM / parse output / execute / write back+yield，附 ASCII 图）+ ⭐ 分歧点 1：动作 3 解析（ToolCallingAgent 拿 HTTP `tool_calls` 字段 vs CodeAgent 从 markdown 抠代码块）+ ⭐ 分歧点 2：动作 4 执行（execute_tool_call 函数调用 + ThreadPoolExecutor 并行 vs python_executor 沙箱）+ 13 字段中 9 个被就地写 + yield 的 4 种事件（ChatMessageStreamDelta / ToolCall / ToolOutput / ActionOutput）+ Week 1 / Day 1-4 完整闭环回收清单 + 给 Day 5 学习者的 3 条提醒（沙箱接口先存疑 / process_tool_calls 重头戏 / `memory_step.tool_calls` ≠ `chat_message.tool_calls`）。**Day 5 之前的认知拼图全部对齐到一处**
- ⭐⭐ 01 [toolcalling-walkthrough.md](03-source/day5-step-stream/01-toolcalling-walkthrough.md) — **小组长李四的一步内部：ToolCallingAgent 单步完整剧本**。延续 Day 4 ③' 张三剧组 + 新增 1 个人物（小组长李四 = `_step_stream`）+ 2 间储物间（活页夹 = `memory_step` 字段，储物柜 = `agent.state`）。主线剧本：搜巴黎天气并转华氏，**10 幕逐个标注变量值 / yield 出去什么 / 老板对讲机响几下**：第 0 幕李四接到半空工作记录 → 第 1 幕翻笔记本 → 第 2 幕打电话问顾问 → 第 3 幕读懂回话 → 第 4a 幕预告 ToolCall → 第 4b 幕 execute_tool_call 4 小步 → 第 4c 幕反馈 ToolOutput → 第 4d 幕写回活页夹 → 第 5 幕 yield ActionOutput 下班。番外篇 5 个（final_answer 特判 / 并行多 tool call ThreadPoolExecutor + copy_context / state 储物柜跨步传图片 / stream_outputs=True 第 2 幕 / **两阶段 yield 设计意图**）+ 📋 附录 A：项目经理张三 3 个怪癖（类构造）+ ❓ 5 个新手 FAQ + 🔗 跟 Day 1-4 + Week 1 完整闭环回收 8 处 + 8 个单步调试断点对照表。**ToolCallingAgent 单篇全覆盖（演出 + 设计意图 + FAQ + 闭环 + 调试），不再单独留 line-by-line 笔记**
- ⭐⭐ 02 [codeagent-walkthrough.md](03-source/day5-step-stream/02-codeagent-walkthrough.md) — **小组长李四的另一种活法：CodeAgent 单步完整剧本**。同任务同剧组，李四这次换 Code 风格的活法 + 新增 1 个人物（代码助手老沙 = `python_executor`）+ 1 件道具（沙箱小屋 namespace）。主线剧本 8 幕（CodeAgent 通常 1 步搞定）：第 0 幕李四接工作记录 → 第 1 幕翻笔记本（同 01）→ 第 2 幕问顾问（⭐ 3 处差异：不带 tools 名片 + `</code>` stop + 手动补 closing tag）→ 第 3 幕 parse_code_blobs 3 把刀 + fix_final_answer_code 修补 → 第 4 幕派老沙跑代码（沙箱接口预告 Day 6）→ 第 5 幕 yield ActionOutput。番外 4 个（structured_outputs JSON 模式 / parse_code_blobs 3 把刀 / fix_final_answer_code 修补 / ⭐ **沙箱 namespace vs 储物柜 跨步状态对比**）+ 跟 01 的对比表（10 vs 8 变化点 / 3 下 vs 2 下对讲机响 / code_action 字段独有 / action_output 谁写差异）+ 老板视角事件序列 + 8 个单步调试断点对照表 + ⭐ **专题：控制 LLM 输出的 3 条路径**（function calling vs structured output vs 自由文本）。**CodeAgent 单篇全覆盖**
